...
namespace caffe {
  ...
  void ConvLSTMReLULayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  template <typename Dtype>
  void ConvLSTMReLULayer<Dtype>::LayerSetUp( const std::vector<Blob<Dtype>*>& bottom, const std::vector<Blob<Dtype>*>& top) {
  ...
  template <typename Dtype>
  void ConvLSTMReLULayer<Dtype>::RecurrentInputBlobNames( std::vector< std::string > * names ) const {
    names->resize(2);
    names->at(0) = "h_t=0";
    names->at(1) = "c_t=0";
  }
  ...
  template <typename Dtype>
  void ConvLSTMReLULayer<Dtype>::RecurrentOutputBlobNames( std::vector<std::string> * names ) const {
    names->resize(2);
    (*names)[0] = "h_t=T";
    (*names)[1] = "c_t=T";
  }
  ...
  template <typename Dtype>
  void ConvLSTMReLULayer<Dtype>::RecurrentInputShapes( std::vector<BlobShape> * shapes ) const {
  ...
  template <typename Dtype>
  void ConvLSTMReLULayer<Dtype>::OutputBlobNames( std::vector<std::string> * names ) const {
  ...
  template<typename Dtype>
  void ConvLSTMReLULayer<Dtype>::FillUnrolledNet( NetParameter * net_param ) const {
    auto param = this->layer_param_.recurrent_param();

    auto debug_param = this->layer_param_.lstm_debug_param();

    std::cout << "Unrolling ConvLSTMReLU" << std::endl;
    ...
    for( int t = 1; t <= this->T_; ++t ){
      ...
      std::cout << "Building ConvLSTMReLUUnit layer" << std::endl;
      ...
      lstm_unit_param->set_type( "ConvLSTMReLUUnit" );
  ...
  INSTANTIATE_CLASS(ConvLSTMReLULayer);
  REGISTER_LAYER_CLASS(ConvLSTMReLU);
