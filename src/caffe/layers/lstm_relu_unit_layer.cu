...
#include "caffe/layers/lstm_relu_layer.hpp"
...
template <typename Dtype>
__global__ void LSTMReLUActsForward(const int nthreads, const int dim,
...
template <typename Dtype>
__global__ void LSTMReLUUnitForward(const int nthreads, const int dim,
...
template <typename Dtype>
void LSTMReLUUnitLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  LSTMReLUActsForward<Dtype><<<CAFFE_GET_BLOCKS(X_count), CAFFE_CUDA_NUM_THREADS>>>(
      X_count, hidden_dim_, X, X_acts);
  CUDA_POST_KERNEL_CHECK;
  // NOLINT_NEXT_LINE(whitespace/operators)
  LSTMReLUUnitForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      count, hidden_dim_, C_prev, X_acts, cont, C, H);
  CUDA_POST_KERNEL_CHECK;
}

template <typename Dtype>
__global__ void LSTMReLUUnitBackward(const int nthreads, const int dim,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    *o_diff = H_diff[index] * relu_c;
    *g_diff = c_term_diff * i;
  }
}

template <typename Dtype>
__global__ void LSTMReLUActsBackward(const int nthreads, const int dim,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    if (d < 3 * dim) {
      ...
      X_diff[index] = X_acts_diff[index] * (X_act > 0);
    }
  }
}

template <typename Dtype>
void LSTMReLUUnitLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  LSTMReLUUnitBackward<Dtype>  // NOLINT_NEXT_LINE(whitespace/operators)
  ...
  LSTMReLUActsBackward<Dtype>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<CAFFE_GET_BLOCKS(X_count), CAFFE_CUDA_NUM_THREADS>>>(
      X_count, hidden_dim_, X_acts, X_acts_diff, X_diff);
  CUDA_POST_KERNEL_CHECK;
}

INSTANTIATE_LAYER_GPU_FUNCS(LSTMReLUUnitLayer);
