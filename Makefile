...
ifeq ($(BLAS), mkl)
	...
	ifeq ($(LINUX), 1)
		ifeq ($(BLAS), atlas)
			...
			#LIBRARIES += cblas atlas #ORIGINAL FILE
			LIBRARIES += satlas #HANH MODIFICATION
		endif
	else ifeq ($(OSX), 1)
...
MATLAB_CXXFLAGS := $(CXXFLAGS) -fopenmp
...
#.PHONY: all lib test clean docs linecount lint lintclean tools examples $(DIST_ALIASES) \
	...
	################ H add from caffe branch contain permute_layer ############
	# ------------ for fopenmp (because in permute_layer, there's #pragma omp parallel for that the compiler ignore when using provided Makefile)
	# COMPILING WITHOUT THE IGNORING MESSAGE
.PHONY: all lib test clean docs linecount lint lintclean tools examples $(DIST_ALIASES) \
	py mat py$(PROJECT) mat$(PROJECT) proto runtest \
...
$(MAT$(PROJECT)_SO): $(MAT$(PROJECT)_SRC) $(STATIC_NAME)
	...
	$(Q) $(MATLAB_DIR)/bin/mex $(MAT$(PROJECT)_SRC) \
			CXX="$(CXX)" \
			CXXFLAGS="\$$CXXFLAGS $(MATLAB_CXXFLAGS)" -lgomp \
