...
void LSTMReLULayer<Dtype>::RecurrentInputBlobNames(vector<string>* names) const {
  names->resize(2);
  (*names)[0] = "h_0";
  (*names)[1] = "c_0";
}
...
template <typename Dtype>
void LSTMReLULayer<Dtype>::RecurrentOutputBlobNames(vector<string>* names) const {
  names->resize(2);
...
template <typename Dtype>
void LSTMReLULayer<Dtype>::RecurrentInputShapes(vector<BlobShape>* shapes) const {
...
template <typename Dtype>
void LSTMReLULayer<Dtype>::OutputBlobNames(vector<string>* names) const {
  names->resize(1);
  (*names)[0] = "h";
}

template <typename Dtype>
void LSTMReLULayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  const int num_output = this->layer_param_.recurrent_param().num_output();
  ...
  for (int t = 1; t <= this->T_; ++t) {
    ...
    //         h_t := o_t .* \tanh[c_t]
    {
      ...
      lstm_unit_param->set_type("LSTMReLUUnit");
...
INSTANTIATE_CLASS(LSTMReLULayer);
REGISTER_LAYER_CLASS(LSTMReLU);
