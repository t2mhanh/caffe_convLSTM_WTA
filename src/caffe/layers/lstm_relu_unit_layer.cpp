...
template <typename Dtype>
void LSTMReLUUnitLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void LSTMReLUUnitLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  for (int n = 0; n < num; ++n) {
    for (int d = 0; d < hidden_dim_; ++d) {
      ...
      const Dtype g = std::max(X[3 * hidden_dim_ + d], Dtype(0));
...
template <typename Dtype>
void LSTMReLUUnitLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(LSTMReLUUnitLayer);
#endif

INSTANTIATE_CLASS(LSTMReLUUnitLayer);
REGISTER_LAYER_CLASS(LSTMReLUUnit);
