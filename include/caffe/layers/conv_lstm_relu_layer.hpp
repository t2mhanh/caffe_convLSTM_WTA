#ifndef CAFFE_CONV_LSTM_RELU_LAYER_HPP
#define CAFFE_CONV_LSTM_RELU_LAYER_HPP
...
namespace caffe{
  ...
  class ConvLSTMReLULayer : public RecurrentLayer<Dtype> {
    public:
      explicit ConvLSTMReLULayer( const LayerParameter & param )
  ...
  template <typename Dtype>
  class ConvLSTMReLUUnitLayer : public Layer<Dtype> {
    public:
      explicit ConvLSTMReLUUnitLayer(const LayerParameter& param)
          : Layer<Dtype>(param) {}
      virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
          const vector<Blob<Dtype>*>& top);

      virtual inline const char* type() const { return "ConvLSTMReLUUnit"; }
