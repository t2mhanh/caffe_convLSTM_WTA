...
% or you can use
caffe.set_mode_cpu();
net_model = ['examples/1_H_examples/UCSD_conv_lstm/ucsd_wta_autoencoder3_test_128d.prototxt'];
% net_weights = ['/usr/not-backed-up/1_convlstm/ucsd_rmsProp/wta_autoencoder_iter_120000.caffemodel'];
net_weights = ['/usr/not-backed-up/1_convlstm/ucsd_rmsProp_128d/wta_autoencoder_iter_70000.caffemodel'];
...
for r = 1:nr
    for c = 1:nc
        W_arrow(stride*(r-1)+2:stride*(r-1)+filSize+1,stride*(c-1)+2:stride*(c-1)+filSize+1,:) = 5*weights_deconv(:,:,:,chosen(id));
        id = id + 1;    
    end
end
figure
